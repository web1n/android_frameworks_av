...
status_t CameraProviderManager::getCameraCharacteristicsLocked(const std::string &id,
    ...
    if (deviceInfo != nullptr) {
        ...
        if (kLegacyTorchStrength != nullptr && id == kLegacyTorchStrength->cameraId) {
            ALOGI("%s: legacyTorchStrength: Getting camera characteristics", __FUNCTION__);

            res = characteristics->update(ANDROID_FLASH_INFO_STRENGTH_DEFAULT_LEVEL,
                &kLegacyTorchStrength->defaultStrength, 1);
            res = characteristics->update(ANDROID_FLASH_INFO_STRENGTH_MAXIMUM_LEVEL,
                &kLegacyTorchStrength->maxStrength, 1);
        }
